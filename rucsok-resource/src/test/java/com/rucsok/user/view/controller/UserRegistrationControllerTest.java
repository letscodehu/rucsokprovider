...
@ContextConfiguration(classes = { UserRegistrationConfig.class, UserRegistrationController.class })
public class UserRegistrationControllerTest {

	@Autowired
	private UserRegistrationTransformer mockTransformer;
	...
	@Test
	@Ignore
	public void registerShouldReturnErrorWithNullParameter() {
	...
	@Test
	@Ignore
	public void registerShouldReturnError_When_UserExists() {

		// GIVEN

		final UserRegistrationRequest request = new UserRegistrationRequest(email, username, password, password);
		Mockito.when(mockService.registerUser(Mockito.any(UserRegistration.class)))
				.thenThrow(new UserAlreadyPresentException());
		...
		Mockito.verify(mockService).registerUser(Mockito.any(UserRegistration.class));
	}

	@Test
	@Ignore
	public void registerShouldReturnTheUserInTheResponse() {

		// GIVEN
		
		final UserRegistrationRequest request = new UserRegistrationRequest(email, username, password, password);
		final User user = new User(email, username);
		UserRegistration mockRegistration = Mockito.mock(UserRegistration.class);
//		UserRegistrationResponse mockResponse = Mockito.mock(UserRegistration.class);
		
		Mockito.when(mockTransformer.transformToRegistration(request)).thenReturn(mockRegistration);
//		Mockito.when(mockTransformer.transformToResponse(user)).thenReturn(mockRegistration);
		Mockito.when(mockService.registerUser(mockRegistration)).thenReturn(user);
	...
	@Test
	@Ignore
	public void registerShouldReturnTheFirstErrorOnValidationErrors() {
